  ...
  return (
    <div className='min-h-screen bg-white'>
      <div className="bg-gradient-to-b from-sky-400 to-white">
        <h1 className="text-xl font-semibold mb-4 text-center">Subject Wise Summary</h1>
      </div>
...
{/* 
        ...
        <div className="mb-4 flex ">
          <h1 className='font-bold font-mono' >Total Collection in Cash: {filteredData.totalCollection.totalCash.toFixed(2)}</h1>
          <h1 className='font-bold font-mono' >Total Collection in Online: {filteredData.totalCollection.totalOnline.toFixed(2)}</h1>
        </div>

        <table className="min-w-full bg-white border border-gray-200">
          <thead>
            <tr>
              <th className="border-b px-4 py-2 text-left text-sm font-semibold">Subject Name</th>
              <th className="border-b px-4 py-2 text-left text-sm font-semibold">Cash Received</th>
              <th className="border-b px-4 py-2 text-left text-sm font-semibold">Online Payment</th>
            </tr>
          </thead>
          <tbody>
            {filteredData.result.map((data, index) => (
              <tr key={index}>
                <td className="border-b px-4 py-2 text-sm">{data.subjectName}</td>
                <td className="border-b px-4 py-2 text-sm">{data.mode === 'Cash' ? data.amount : '-'}</td>
                <td className="border-b px-4 py-2 text-sm">{data.mode === 'Online' ? data.amount : '-'}</td>
              </tr>
            ))}
          </tbody>
        </table>
